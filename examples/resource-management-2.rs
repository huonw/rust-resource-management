...
There's three things to do here, all in and around the `main` function below:

1. open the `data/examples.csv` file with `open_file`, and get a sense of when it gets closed. Feel
   free to define your own functions that take a `file: File` argument (and return them), and
...
// helpful. This attribute (`#`) is attached to this whole file (`!`), and "allows" the `dead_code`
// lint in the compiler, so that it doesn't emit any warnings. (Feel free to comment out or remove
// this line, to see what the compiler can flag!)
#![allow(dead_code)]
