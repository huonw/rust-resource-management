...
fn split_headings_example_csv(headings_line: String) -> Vec<String> {
    ...
    // this is a macro that can be any type, but panics the program if it is executed, similar to
    // `raise NotImplementedError()` in Python
    unimplemented!()
}
...
fn main() {
    ...
    let headings: Vec<String> = split_headings_example_csv(headings_line);
